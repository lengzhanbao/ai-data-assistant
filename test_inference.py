...
t("power: adequate for d=0.8 n=30", rp.get("adequate"))
rp2 = power_analysis(n=10, effect_size=0.2)
t("power_low_n: not adequate", not rp2.get("adequate"))
...
t("regdiag: no high VIF", not rd.get("high_vif_warning") or len(rd.get("high_vif_vars",[])) == 0)
...
# Edge cases
try:
    bayes_alternative(np.array([]), np.array([]))
    t("bayes_empty: no crash", True)
except Exception:
    t("bayes_empty: no crash", False)

try:
    robustness_check(pd.DataFrame({"val": [np.nan] * 10}), "val")
    t("robust_nan: no crash", True)
except Exception:
    t("robust_nan: no crash", False)

try:
    regression_diagnostics(pd.DataFrame({"x": [1], "y": [1]}), "y", ["x"])
    t("reg_tiny: no crash", True)
except Exception:
    t("reg_tiny: no crash", False)
